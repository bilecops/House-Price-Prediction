...
@st.cache_data
def download_and_load_model(url, output_name):
    gdown.download(url, output_name, quiet=False)
    return joblib.load(output_name)

# Load models
landed_model = download_and_load_model("https://drive.google.com/uc?id=1zjm47dBN6TdcqLZOu8F4fSgZo2xYTjYn", "RandomForest_Landed.joblib")
highrise_model = download_and_load_model("https://drive.google.com/uc?id=1Ygl5uDZGndK7B_DG0Rdr71Sv9NSouV3r", "RandomForest_Highrise.joblib")

# Rest of the Streamlit app code continues...
...
feature3 = st.date_input("Landed Transaction Date", value=datetime.datetime(2020, 1, 1))
...
if st.button("Predict Landed Price"):
    ...
    # Perform prediction
    predicted_price = landed_model.predict(input_features)[0]
...
feature1a = st.number_input("Highrise Log Land/Parcel Area", value=4.5, step=0.1)
...
feature3a = st.date_input("Highrise Transaction Date", value=datetime.datetime(2020, 1, 1))
...
if st.button("Predict Highrise Price"):
    ...
    # Perform prediction
    predicted_price_h = highrise_model.predict(input_featuresa)[0]
